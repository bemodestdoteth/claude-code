    ...
    async getPromptForCommand(args: string, context: ToolUseContext) {
      if (!getClaudeAIOAuthTokens()?.accessToken) {
        return [
          {
            type: 'text',
            text: 'You need to authenticate with a Claude.ai account first. API accounts are not supported. Configure Claude.ai account authentication, then try /schedule again.',
          },
        ]
      }
