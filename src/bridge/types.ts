...
/** Reusable authentication guidance appended to bridge auth errors. */
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is only available with Claude.ai subscriptions. Configure Claude.ai account authentication before using Remote Control.'

/** Full error printed when `claude remote-control` is run without auth. */
export const BRIDGE_LOGIN_ERROR =
  'Error: You must be authenticated to use Remote Control.\n\n' +
