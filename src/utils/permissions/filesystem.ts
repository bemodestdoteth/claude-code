...
export function checkReadPermissionForTool(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'deny',
        message: `Permission to read ${path} has been denied because it appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'deny',
        message: `Permission to read ${path} has been denied because it contains a suspicious Windows path pattern.`,
        decisionReason: {
          type: 'other',
          reason:
            'Path contains suspicious Windows-specific patterns (alternate data streams, short names, long path prefixes, or three or more consecutive dots)',
        },
      }
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Claude requested permissions to read ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
  ...
  // 8. Check for allow rules - check both the original path and resolved symlink path
  for (const pathToCheck of pathsToCheck) {
  ...
  // 12. Read permissions hard-cutover: ordinary reads are allowed without a prompt.
  // Explicit deny rules and suspicious path checks above still block unsafe reads.
  return {
    behavior: 'allow',
    updatedInput: input,
    decisionReason: {
      type: 'workingDir',
      reason: 'Read permissions do not require confirmation',
    },
  }
}
